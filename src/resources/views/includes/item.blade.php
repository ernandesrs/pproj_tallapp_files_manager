...
<div
    class="relative overflow-hidden col-span-6 sm:col-span-4 md:col-span-3 xl:col-span-2 flex flex-col gap-x-2 items-center border dark:border-zinc-700 duration-200 px-5 py-3 rounded cursor-default {{ $file ? 'bg-zinc-50 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700' : 'bg-zinc-300 dark:bg-zinc-900 hover:bg-zinc-200 dark:hover:bg-zinc-800' }}"
    {{ $attributes }}>

    @if (!$show)
        <div
            class="absolute z-10 top-0 left-0 opacity-75 bg-zinc-300 dark:bg-zinc-900 w-full h-full flex justify-center items-center">
            <x-icon name="lock" class="w-8 h-6 lg:w-12 lg:h-10" />
        </div>
    @else
        <div
